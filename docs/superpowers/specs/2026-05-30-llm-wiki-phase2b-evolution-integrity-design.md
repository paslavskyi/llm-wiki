...
4. **Сирітки** — конкретні нотатки (НЕ `topic`) без жодного `links` і без
   ...
   сиріток).
5. **Ймовірні дублі** — нотатки одного `type` зі схожими `title`/`summary`
   (евристика; напр. нормалізований Левенштейн/Жаккар по словах). **Вимкнено за
   замовчуванням**, вмикається прапорцем `--duplicates` (найшумніший сигнал).
...
(backlinks), і на які посилається X, на N кроків (default — закладемо 1, з
прапорцем глибини). Використовує `index/backlinks.json` (вже генерується). Чиста
функція `computeImpact(id, notes, depth)` + CLI. Споживається `kb-evolve` перед
змінами.
...
tools/hooks/pre-commit             # NEW — versioned pre-commit source (+install doc)
...
- **`graph.mjs` (юніт):** кожен Tier-1 сигнал (борг міграції; відкриті
  Q/RISK/ASMP; порожні topic-вузли); Tier-2 сирітки; дублі лише з `--duplicates`;
  завжди exit 0; health.md має банер.
...
- `Q-2B-001`: евристика «ймовірних дублів» — конкретний алгоритм/поріг
  (нормалізований Жаккар по токенах title+summary?). Визначити при реалізації
  `graph.mjs`; за замовчуванням вимкнено, тож не блокує.
- `Q-2B-002`: глибина `impact.mjs` за замовчуванням (закладаю 1; прапорець для N).
- `Q-2B-003`: чи додавати npm-скрипти `graph`/`impact`/`heal` у package.json
  (ймовірно так, для зручності; вирішити при реалізації).
- `Q-2B-004`: інсталяція pre-commit — документувати ручний `cp tools/hooks/pre-commit
  .git/hooks/` чи додати `npm run install-hooks`. Схиляюсь до npm-скрипта.
